...
namespace App
{
    ...
    {
        public static void EnsureContainer(this CloudBlobContainer cloudBlobContainer,
        ...
        /// <returns></returns>
        public static string GetContainerSharedAccessSignatureUri(this CloudBlobContainer container,
            string policyName = null, TimeSpan? duration = null,
            SharedAccessBlobPermissions sharedAccessBlobPermissions = SharedAccessBlobPermissions.Read)
        {
            ...
            var containerSharedAccessSignatureToken =
            ...
            var result = container.Uri + containerSharedAccessSignatureToken;
            return result;
        }
...
//Set the expiry time and permissions for the container.
        ...
        public static void UploadText(this CloudBlobContainer cloudBlobContainer, string remoteBlobName, string text)
        {
            ...
            var blob = cloudBlobContainer.GetBlockBlobReference(remoteBlobName);
            blob.UploadText(text);
        }

        public static void UploadFile(this CloudBlobContainer cloudBlobContainer, string localFileName,
            string remoteBlobName = null)
        {
        ...
        public static void UploadStream(this CloudBlobContainer cloudBlobContainer, string remoteBlobName,
            Stream stream)
        {
            //Note that this is a BlockBlob, nut just a blob:
            var blob = cloudBlobContainer.GetBlockBlobReference(remoteBlobName);
        ...
        public static byte[] DownloadStream(this CloudBlobContainer cloudBlobContainer, string remoteBlobName)
        {
            ...
            var downloaded = blob.DownloadToByteArray(result, 0);
            return result;
        }

        public static Stream DownloadStream(this CloudBlobContainer cloudBlobContainer, string remoteBlobName,
            Stream stream = null)
        {
            var blob = cloudBlobContainer.GetBlockBlobReference(remoteBlobName);
            if (stream == null)
        ...
        public static bool DeleteBlob(this CloudBlobContainer cloudBlobContainer, string remoteBlobName)
        {
            ...
            var blobSource = cloudBlobContainer.GetBlockBlobReference(remoteBlobName);
            var blobExisted = blobSource.DeleteIfExists();
            return blobExisted;
        }

        public static string[] ListContainerContents(this CloudBlobContainer cloudBlobContainer,
            string pathPrefixFilter = null, bool removeContainerName = true)
        {
            ...
            string[] results = cloudBlobContainer.ListBlobs(pathPrefixFilter, true, BlobListingDetails.All)
                .Select(x => removeContainerName ? GetFileNameFromBlobURI(x.Uri, containerName) : x.Uri.ToString())
                .ToArray();
        ...
        {
            var path = theUri.ToString();
            var dirIndex = path.IndexOf(containerName);

            var result = path.Substring(
